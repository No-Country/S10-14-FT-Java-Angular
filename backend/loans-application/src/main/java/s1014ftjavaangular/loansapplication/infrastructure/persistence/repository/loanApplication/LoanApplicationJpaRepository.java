...
public interface LoanApplicationJpaRepository extends JpaRepository<LoanApplicationEntity, String> {
    ...
    @Query(value = """ 
            ...
            WHERE (la.status = 'INCOMPLETE' OR la.status = 'AUDITING') AND gd.identification = :identification AND la.customer_id = :customerId
            """, nativeQuery = true)
    Integer countIncompleteOrAuditingStatusLoanApplication(String identification, String customerId);
}
